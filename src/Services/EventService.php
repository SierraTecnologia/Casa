...
class EventService
{
    ...
    {
        Carbon::setLocale(\RicardoSierra\Translation\Services\LanguageService::getSingleton()->getActualLanguage());
    ...
    public function links($class = null)
    {
        ...
        $links = '';
        $links .= '<a class="previous '.$class.'" href="'.URL::to('events/'.$previousMonth).'">'.trans('features.previousMonth').'</a>';
        $links .= '<a class="next '.$class.'" href="'.URL::to('events/'.$nextMonth).'">'.trans('features.nextMonth').'</a>';
